...
export const Navbar: React.FC = () => {
  ...
  // Funciones de autenticación conectadas a Firebase
  ...
  const authItems = isAuthenticated
    ? [
        { name: 'Perfil', path: '/profile', icon: <UserIcon className="w-5 h-5" /> },
        { name: 'Cerrar Sesión', action: handleLogout, icon: <LogInIcon className="w-5 h-5" /> }
      ]
    : [
        { name: 'Iniciar Sesión', action: handleLogin, icon: <LogInIcon className="w-5 h-5" /> }
      ];
              ...
              {authItems.map((item) =>
                item.path ? (
                  ...
                  <button
                    key={item.name}
                    onClick={item.action}
                    className="px-3 py-2 rounded-md text-sm font-medium flex items-center space-x-1 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
                  >
                    {item.icon}
                    <span>{item.name}</span>
                  </button>
